...
password = getpass.getpass("Enter the the elevenlabs api key")
...
def translate_description(descriptions, target_language):
    ...
    return parsed_translation['Translation'], parsed_translation["Sentences"][0], parsed_translation["Sentences"][1], parsed_translation["Sentences"][2]
...
llm_name = "alpindale/Llama-3.2-3B-Instruct"
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            translated_sentence_1 = gr.Textbox(label="Sample Sentence 1")
            translated_sentence_2 = gr.Textbox(label="Sample Sentence 2")
            translated_sentence_3 = gr.Textbox(label="Sample Sentence 3")

            vocalize_button_word = gr.Button(f"Vocalize Word")
            vocalize_button_1 = gr.Button(f"Vocalize Sentence 1")
            vocalize_button_2 = gr.Button(f"Vocalize Sentence 2")
            vocalize_button_3 = gr.Button(f"Vocalize Sentence 3")
    ...
    translate_button.click(
        ...
        outputs=[translation_output, translated_sentence_1, translated_sentence_2, translated_sentence_3],
    )

    vocalize_button_word.click(
        vocalize_text,
        inputs=[translation_output],
        outputs=[]  # No need to return anything, just vocalizing the text
    )

    vocalize_button_1.click(
        vocalize_text,
        inputs=[translated_sentence_1],
        outputs=[]  # No need to return anything, just vocalizing the text
    )
    
    vocalize_button_2.click(
        vocalize_text,
        inputs=[translated_sentence_2],
        outputs=[]  # No need to return anything, just vocalizing the text
    )
    
    vocalize_button_3.click(
        vocalize_text,
        inputs=[translated_sentence_3],
        outputs=[]  # No need to return anything, just vocalizing the text
    )
